...
# TODO: fix this to use couples' rates where applicable, and to cap premium at two children.
...
ggplot(results) + geom_histogram(aes(x=fam.net.capped), binwidth=100) +geom_vline(aes(xintercept=fam.net.max),color='red')+ facet_wrap( ~ plan.name, ncol=1)
...
p.dist <- ggplot(results, aes(x=fam.net.capped)) +
          ...
          scale_x_continuous("Net family costs", limits=c(0, 30000), labels = dollar)
